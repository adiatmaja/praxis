...
- **Capability decision records live in `core/capability_events.py`** (S1: versioned Pydantic events + `capability_events` table + bus wiring); `Orchestrator.__init__` constructs `self._emitter = CapabilityEventEmitter(task_queue._db, event_bus)` unconditionally, and `ReviewMixin._run_leaf_triage` emits `TaskSplitEvent`/`TaskEscalatedEvent` through it.
