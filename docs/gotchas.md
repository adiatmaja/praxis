...
## The MCP configuration block was printed once and could not be recovered

`praxis init` prints the MCP client configuration block that the README's agent setup
brief tells an assistant to paste into `.mcp.json`. Until 2026-08-28 that was the ONLY
producer:
...
So the block appeared once, in the middle of an output that is mostly Docker build
progress and takes minutes to produce, and an operator whose scrollback had rolled had to
re-run the entire install to see it again. Found by running the brief for real: a
subagent given a fresh clone and the brief lost the block to output truncation and
reported that its only remaining route was to run `init` again. Step 5 of that brief
consists of pasting the block, so the step was effectively un-completable from a long
transcript.

`praxis mcp` re-prints it. Four properties, each of which was a way to get it wrong:

- **it must PARSE.** The longest line is the absolute install path inside a JSON string,
  so a rich fold emits invalid JSON with a broken path. Hence `soft_wrap=True` here too;
- **`--directory` must be the INSTALL ROOT, not the cwd.** `mcp_snippet` defaults to
  `Path.cwd()`, which is correct for `init` (it has already proven the cwd is the root)
  and wrong for a verb meant to be runnable from a subdirectory: a `--directory` pointing
  at `src/orchestrator` hands the client a `uv run` that cannot find the project. The root
  is taken from the directory holding the `.env` the CLI resolved;
- **it must work with the orchestrator DOWN**, like `praxis presets`, because setting an
  MCP client up is exactly when the server may not be running. The guard makes `httpx`
  explode to prove the happy path never builds a client;
- **the snippet comes from `cli.init.mcp_snippet`**, never a second copy. The env var
  names in it are the ones `mcp_server.client` actually reads, and a block with
  plausible-but-wrong names is worse than no block: it gets pasted, and the server then
  silently falls back to a default URL with no token.

## A high-entropy credential is invisible to the secret scan

The only real credential ever committed to this repository was a `secrets.token_urlsafe(32)`
`AUTH_TOKEN`, generated by a throwaway cold install and left in a test fixture on
2026-08-28. **gitleaks ran over it and was GREEN.**

The rules in `.gitleaks.toml` match SHAPED secrets: a `ghp_` prefix, `sk-`, `AKIA`, a PEM
header. A URL-safe base64 blob has no shape to match, so the scan cannot see it. That is
a property of the tool, not a misconfiguration, and it means **a green Security workflow
is not evidence that no credential was committed** when the credential in question is one
this product generates itself.
