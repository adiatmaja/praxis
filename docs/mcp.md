...
| `wait_task(task_id, timeout_seconds=90)` | **Wait, do not poll.** Blocks on the server until the task's status changes, the task comes to rest (terminal, or parked on a person), or the timeout (capped at 90 s) passes. Returns the state reached, `changed` / `timed_out`, `waiting_on` (`worker`, `review`, `human`, `nothing`) and `next_action`: `wait_again`, `relay_pr`, `answer_clarification`, `retry` or `none`. A task at a human gate returns at once with `changed=false`, and so does a `pending` leaf behind a gated or failed dependency (`blocked_by` names it). Expected durations: a small leaf on Gemini Flash Low finishes in under a minute; review is a brain call of a minute or two. CLI twin: `praxis wait <task-id>`. |
...
| `pending_approvals()` | Everything waiting on a human, across ALL projects and all THREE gates: `tasks` (reviewed PRs at the merge gate), `plans` (completed plans whose integration PR is open), `proposals` (autonomous improvement plans nobody has approved to run) and `clarifications` (tasks blocked on a question). `count` covers only the first two, and counts DISTINCT PULL REQUESTS rather than rows, because it is rendered as a number of pull requests: single-branch mode puts N tasks on one shared PR, so `count` can be smaller than `task_count + plan_count`. Read `summary` for the whole queue. This is the queue that actually has to be cleared. |
...
| `retry_task(task_id)` | Requeue a FAILED task: back to `pending` with `attempt + 1`. Returns `{summary, task_id, status, attempt, plan_id}`. This is the action `poll_plan`'s `stalled.action_required = "retry_failed_task"` names, and retrying the failed leaf is what makes its dependents dispatchable again. `failed` is the ONLY status it accepts; every other one answers 409, including the three other terminal statuses (`merged`, `no_changes`, `superseded`) which are not failures and have nothing to re-run. The re-run starts clean: the branch is rebuilt from base and the worker's stored session is dropped. There is no cap here, so read `get_task_logs` and change something before calling it again. CLI twin: `praxis retry <task-id>`. |
