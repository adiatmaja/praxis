...
@pytest.mark.unit
async def test_chain_resolves_to_correct_models(
...
async def test_registry_override_wins(db: Database, test_settings: Settings) -> None:
    ...
    assert chain == [
        {"provider": "custom_provider", "model": "custom_m", "effort": "low"}
    ]
...
@pytest.mark.unit
async def test_call_site_override_used_when_no_role_chain(
    ...
    await db.execute(
        ...
        (
            "models.plan_spec",
            json.dumps(
                {"provider": "custom_prov", "model": "custom_mod", "effort": "high"}
            ),
        ),
    )
