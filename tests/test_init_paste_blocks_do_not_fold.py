...
#: Fake, and deliberately the same LENGTH as a real `praxis init` token: 43
#: characters, matching `secrets.token_urlsafe(32)`. Length is the whole point,
#: because it decides whether the export lines fold, so shortening this quietly
#: weakens the guards below. Realism is not required, width is.
#:
#: The first version of this file used the ACTUAL token generated by a
#: throwaway cold install. It was dead and had only ever bound to 127.0.0.1,
#: but a real credential value does not belong in a public repository.
_FAKE_TOKEN = "EXAMPLE_NOT_A_REAL_TOKEN_aaaaaaaaaaaaaaaaaa"

#: Four characters shorter, for the case that does not need the longest line.
_FAKE_TOKEN_SHORT = "EXAMPLE_NOT_A_REAL_TOKEN_aaaaaaaaaaaaaa"
...
def test_the_mcp_block_is_still_valid_json_at_a_narrow_width(
    ...
    text = _render_next_steps(monkeypatch, _FAKE_TOKEN_SHORT)
